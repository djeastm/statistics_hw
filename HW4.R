...
data1 = c(32, 35, 41, 28, 27)
data2 = c(27, 29, 24, 25, 26)
data_digits = 1
confidence = T 
conf_ratio = .90
median_expl = "median salinity"
...
null_hyp_expl_str = "the same level of salinity at the East and West sites"
alt_hyp_type = "\\neq"
#"There is [evidence measure] of ___________ (p = X.XXXX): "
# -ing tense
alt_hyp_expl_str = "median salinity being different at the two sites"
...
if (as.character(test_type) == "1") {
  ...
  var = "\\theta"
  ##paired <- readline(prompt="Is the data paired or independent?: ")
} else if (as.character(test_type) == "4") {
  var = "\\Delta"
}
...
if (as.character(test_type) == "1") { # in One-Sample t-Test Mode
...
} else if (as.character(test_type) == "4") {# in Wilcoxon Rank Sum Test Mode
  ...
  big.frame = data.frame(1:N)
  delta_0 = null_hyp
  
  big.frame["Data"] = c(data1,data2)
  big.frame["Labels"] = c(rep(data1_label,n),rep(data2_label,m))
  big.frame["Ranks"] = rep(-1,N)
  sorted_big.frame = big.frame[order(big.frame["Data"]),]
  ...
  ties_exist = F
  for(i in 1:nrow(sorted_big.frame)) {
    if (is_tied == F && sorted_big.frame[i,"Data"] == last) {
      is_tied = T
      ties_exist = T
      if (i == nrow(sorted_big.frame)) {
        tie_start = i-1
    ...
    } else if (sorted_big.frame[i,"Data"] == last) {
    } else if (is_tied == T) {
      # set all previous ones to tied average
      avg = ((i-1) + tie_start-1) / 2
      for (k in tie_start:i-1) {
  ...
  test_stat = sum(sorted_big.frame[which(sorted_big.frame[,"Labels"]== data1_label),"Ranks"])
  test_stat_str = paste("W_{c}&=\\sum_{j=1}^{n} S_j=\\sum_{j=1}^{",n,"} S_j = ",test_stat,sep="")
  
  U_test_stat = test_stat - (n*(n+1)/2)
  U_test_stat_str = paste("U_{c}&=W_{c} - \\frac{n(n+1)}{2} = ",test_stat," - \\frac{",n,"(",n,"+1)}{2} = ",U_test_stat,sep="")
  
  calc_test_stat_str = paste("\\begin{align*}",test_stat_str,"\\\\",U_test_stat_str,"\\end{align*}",sep="")
  
  sorted_big.frame = sorted_big.frame[order(sorted_big.frame[,1]),]
  
  display.frame = data.frame(1:m)
  display.frame[data1_label] =  c(sorted_big.frame[which(sorted_big.frame[,"Labels"]== data1_label),"Data"],rep(NA,m-n))
  display.frame["X Rank"] = c(sorted_big.frame[which(sorted_big.frame[,"Labels"]== data1_label),"Ranks"],rep(NA,m-n))
  display.frame[data2_label] =  sorted_big.frame[which(sorted_big.frame[,"Labels"]== data2_label),"Data"]
  display.frame["Y Rank"] = sorted_big.frame[which(sorted_big.frame[,"Labels"]== data2_label),"Ranks"]
  
  display.frame = t(display.frame)
  display.frame = display.frame[-1,]
  
  if (ties_exist ==  T) {
    digits_mat = matrix(c(data_digits,1,data_digits,1),nrow = length(display.frame[,1]), ncol=length(display.frame[1,])+1, byrow=FALSE)
  } else {
    digits_mat = matrix(c(data_digits,0,data_digits,0),nrow = length(display.frame[,1]), ncol=length(display.frame[1,])+1, byrow=FALSE)
  }
  ...
  ## CALCULATE P-VALUE
  if (alt_hyp_type == "<") {
    p_call_str = paste("P(U\\leq U_{c}) &= \\texttt{pwilcox(U\\_c,n,m)}\\\\ &= \\texttt{pwilcox(",U_test_stat,",",n,",",m,")}\\\\",sep="")
    p_val = pwilcox(U_test_stat,n,m)
    alter_param = "less"
  } else if (alt_hyp_type == ">") {
    p_call_str = paste("P(U\\geq U_{c}) &= \\texttt{pwilcox(U\\_c - 1,n,m,lower.tail=F)}\\\\ &= \\texttt{pwilcox(",U_test_stat," - 1,",n,",",m,",lower.tail=F",")}\\\\",sep="")
    p_val = pwilcox(U_test_stat-1,n,m,lower.tail=F)
    alter_param = "greater"
  } else { # \neq
    if (test_stat <= (n*(n+m+1))/2) {
      p_call_str = paste("2*P(U\\leq U_{c}) &= \\texttt{2*pwilcox(U\\_c,n,m)}\\\\ &= \\texttt{2*pwilcox(",U_test_stat,",",n,",",m,")}\\\\",sep="")
      p_val = 2*pwilcox(U_test_stat,n,m)
    } else if ((test_stat > (n*(n+1))/4)) {
      p_call_str = paste("2*P(U\\geq U_{c}) &= \\texttt{2*pwilcox(U\\_c - 1,n,m,lower.tail=F)}\\\\ &= \\texttt{2*pwilcox(",U_test_stat," - 1,",n,",",m,",lower.tail=F",")}\\\\",sep="")
      p_val = 2*pwilcox(U_test_stat-1,n,m,lower.tail=F)
    }
    alter_param = "two.sided"
  }
}
...
if (as.character(test_type) == "1") {
  ...
  library(exactRankTests)
  r_t_test_command_str = paste("\\texttt{> wilcox.exact(data1,data2,alternative=\"",alter_param,"\")}",sep="")
  capt_out = capture.output(wilcox.exact(data1,data2,alternative=alter_param))
  r_output_str = paste("\\texttt{",capt_out,"}\\\\",sep="")
}
...
if (confidence == T) {
  if (as.character(test_type) == "3") {
  ...
  else if (as.character(test_type) == "4") { # Moses CI
    #Compute pairwise differences
    U = sort(outer(data1,data2,'-'))
      
    conf_diff_command_str = paste("\\texttt{> U = sort(outer(data1,data2,'-'))}",sep="")
    conf_diff_str = U #split(swa, ceiling(seq_along(swa)/10))
    
    #Estimator
    estimator = median(U)
    estimator_command_str = paste("\\texttt{> estimator = median(U)}",sep="")
    estimator_str = paste("$$\\hat{\\Delta} = ",round(estimator,data_digits+1),"$$",sep="")
    
    #Confidence Interval
    tail = (1-conf_ratio)/2
    v = qwilcox(tail,n,m)
    conf_int = c(U[v],U[length(U)+1-v])
    conf_int_first = round(conf_int[1],data_digits)
    conf_int_second = round(conf_int[2],data_digits)
    conf_int_command_str = paste("\\texttt{> n = length(data1)}\\\\ \\texttt{> m = length(data2)}\\\\ \\texttt{> tail = (1-",conf_ratio,")/2}\\\\ \\texttt{> v = qwilcox(tail,n,m)} \\\\ \\texttt{> c(U[v],U[length(U)+1-v])} \\\\ \\texttt{",round(conf_int[1],data_digits)," ",round(conf_int[2],data_digits),"}",sep="")
    conf_int_str = paste("The ",conf_ratio*100,"\\% confidence interval is between ", conf_int_first," and ",conf_int_second,".",sep="")
    ...
    cat (item,"Compute all pairwise differences.\n")
    cat("\\par", conf_diff_command_str,"\n")
    cat("\\par ")
    cat(conf_diff_str)
    cat("\n")
    
    cat (item,"Order the differences and find the median difference.\n")
    cat("\\par", estimator_command_str,"\n")
    cat("\\par", estimator_str,"\n")
    
    cat (item,"Find the appropriate percentile from the Wilcoxon distribution.\n")
    cat("\\par", conf_int_command_str,"\n")
    cat("\\par", conf_int_str,"\n")
    
    cat (item,"State and interpret the interval.\n")
    cat("\\par", interpret_str,"\n")
